...
Transport* MapManager::LoadTransportInMap(Map* instance, uint32 goEntry, uint32 period)
{
    ...
    {
        sLog->outError(LOG_FILTER_SQL, "Transport ID:%u, will not be loaded, gameobject_template missing", goEntry, goInfo->name.c_str());
        return NULL;
    }
    ...
    {
        sLog->outError(LOG_FILTER_SQL, "Transport ID: %u, Name: %s, will not be loaded, gameobject_template type wrong", goInfo->moTransport.taxiPathId);
        delete t;
        return NULL;
    }
...
void MapManager::LoadTransportForPlayers(Player* player)
{
    ...
    for (MapManager::TransportSet::const_iterator i = tset.begin(); i != tset.end(); ++i)
    {
        ...
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Transport to Transporter");
    }
...
void MapManager::UnLoadTransportForPlayers(Player* player)
{
    ...
    for (MapManager::TransportSet::const_iterator i = tset.begin(); i != tset.end(); ++i)
    {
        ...
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Unloading Transportation from Transporter");
    }
...
void Transport::Update(uint32 p_diff)
{
    ...
    while (((m_timer - m_curr->first) % m_pathTime) > ((m_next->first - m_curr->first) % m_pathTime))
    {
        ...
        else
        {
			...
			UpdatePlayerPositions(); // Make sure players move with transport
        }
...
Creature* Transport::AddNPCPassengerInInstance(uint32 entry, float x, float y, float z, float o, uint32 anim)
{
    ...
    if (!creature->IsPositionValid())
    {
        sLog->outError(LOG_FILTER_SQL, "Creature (guidlow %d, entry %d) not created. Suggested coordinates isn't valid (X: %f Y: %f)", creature->GetGUIDLow(), creature->GetEntry(), creature->GetPositionX(), creature->GetPositionY());
        delete creature;
        return 0;
    }
